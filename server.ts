...
async function startServer() {
  ...
  app.post("/api/otp/send", async (req, res) => {
    try {
      ...
      console.error("Error in /api/otp/send route handler:", err);
      res.status(500).json({ error: "Failed to dispatch verification code: " + (err?.message || String(err)) });
    }
  });
  ...
  app.post("/api/otp/verify", (req, res) => {
    try {
      ...
      console.error("Error in /api/otp/verify route handler:", err);
      res.status(500).json({ error: "Verification server exception: " + (err?.message || String(err)) });
    }
  });
